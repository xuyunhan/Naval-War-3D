...
public class JOGL06_Texture implements GLEventListener, KeyListener,
	...
	public void camera() {
		...
		// System.out.println("当前的摄像机的坐标 " + g_eye[0] + g_eye[1] + g_eye[2]);
		// System.out.println("当前的摄像机的角度" + g_look[0] + " " +g_look[1] + " " +
		// g_look[2]);
		if (rad_xz > 2.2 && rad_xz < 2.5) {
	//		System.out.println("当前的旋转角" + rad_xz);
		}
	}
	...
	public void Drawcube(GLAutoDrawable drawable) {
		...
		/*
		 * // 左面 gl.glTexCoord2f(0.0f, 0.0f);gl.glVertex3f(5.0f, -5f, -7.0f);
		 * gl.glTexCoord2f(1.0f, 0.0f);gl.glVertex3f(5.0f, -5f, 7.0f);
		 * gl.glTexCoord2f(1.0f, 1.0f);gl.glVertex3f(5.0f, 9f, 7.0f);
		 * gl.glTexCoord2f(0.0f, 1.0f);gl.glVertex3f(5.0f, 9f, -7.0f); /* // 右面
...
//				System.out.println("此时x的增加量 " + Math.cos(rad_xz));
//				System.out.println("此时的旋转的角度是 " + rad_xz);
//				System.out.println("此时的x的坐标 " + x);
//				System.out.println("此时的z的坐标 " + z);
				if ((Math.abs(x - 30)) < 2 && (Math.abs(z + 30)) < 2) {
					System.out.println("此时已经打中目标");
				}
			}
		}
